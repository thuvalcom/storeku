<div class="flex-1 overflow-y-auto overflow-x-hidden bg-gray-200">
    <main class="flex-1 overflow-y-auto overflow-x-hidden bg-gray-200">
        ...
        <div class="container mx-auto px-6 py-8">
            ...
            @if ($updateMode)
                @include('livewire.blog.post-category-update')
            @else
                <a wire:navigate href="{{ route('postcategory.create') }}"
                    class="ml-3 rounded-md bg-teal-600 px-6 py-3 font-medium tracking-wide text-white hover:bg-teal-500">Create
                    new category</a>
                {{-- table --}}
                <div class="mt-8">

                    <h4 class="text-gray-600">All categories</h4>
                    {{-- form search --}}
                                        ...
                                        <tr wire:key="{{ $category->id }}">
                                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                                <div class="flex items-center">
                                                    <div class="ml-3">
                                                        <p class="whitespace-no-wrap text-gray-900">{{ $category->id }}
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                                <p class="whitespace-no-wrap text-gray-900">{{ $category->name }}</p>
                                            </td>
                                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                            ...
                                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                                <p class="whitespace-no-wrap text-gray-900">{{ $category->created_at }}
                                                </p>
                                            </td>
                                            <td class="border-b border-gray-200 bg-white px-5 py-5 text-sm">
                                                <button wire:click="edit({{ $category->id }})"
                                                    class="ml-3 rounded-md bg-blue-600 px-6 py-3 font-medium tracking-wide text-white hover:bg-blue-500">
                                                    Edit
                                                </button>
                                                <button wire:click="confirmDelete({{ $category->id }})"
                                                    class="ml-3 rounded-md bg-red-600 px-6 py-3 font-medium tracking-wide text-white hover:bg-red-500">
                                                    Delete
                                                </button>
    ...
    <!-- Modal -->
    @if ($confirmingDelete)
        <div class="fixed inset-0 flex items-center justify-center bg-slate-900 bg-opacity-80">
            <div class="rounded bg-indigo-800 p-6 shadow-md">
                <p class="text-lg text-white">Are you sure, you want to delete {{ $categoryToDelete->name }}?</p>
